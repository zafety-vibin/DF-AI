...
- **State** (`tools_state.go`): status, alerts, dwarves, and plugin-query proxies (dwarf detail, stocks, jobs, orders, a placed-buildings listing). Stocks carry material names and an economic-stone flag alongside item type and count. Alerts are DF's own announcement stream — the primary "why isn't this working" signal. `job_types` is a separate, on-demand lookup of DFHack's full job_type enum (filterable by name substring) for `queue_job`'s generalized name-based path — not folded into `orders`, to keep that cheap on every call.
...
- **Action** (`tools_action.go`): designations (digging, tree-chopping, plant-gathering), buildings, zones, stockpiles, orders, blueprints. `queue_job` queues a single job directly at an existing workshop — no manager noble or office required, unlike `order`; its item vocabulary falls back to any DFHack job_type name (found via `job_types`) when the short built-in list doesn't cover it. `set_labor` enables/disables one labor on a dwarf directly (DF's own job-assignment AI then picks up matching work — this doesn't queue a job itself). **Truthful ACKs**: the plugin's error text reaches the model verbatim as SUCCESS/PARTIAL/FAILED — never soften it. Workshop coordinates are model-facing CENTER, converted once to the wire's NW corner (`buildWireCoords`).

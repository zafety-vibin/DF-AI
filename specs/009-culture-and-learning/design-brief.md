...
Predicates today measure dug tiles and counts. Expand with the data now flowing: food/drink stocks quantified (stockpile_inventory → predicates), mood/stress risk (dwarf_detail), defense posture (entrance sealed?). `check_goals` becomes the model's habitual self-critic (charter already mandates it; make its output goal-diff-aware: "changed since last check"). A dwarf-count corruption bug already found and fixed pre-009 (2026-07-12: tame animals were misclassified as citizens, inflating every per-dwarf predicate's denominator) — trust `EntitySnapshot.Dwarves` directly, no need to re-derive a citizen filter here.
